...
and supports `--reset` to run `soft_reset_device()` (issues `default interface` + `no router` to clear running-config state without a reload) before pushing configs, and `--node` to target a single device.
