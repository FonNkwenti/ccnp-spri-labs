...
# Pre-flight: verify the RP address in the running config before injecting.
# 'show run | include ip pim rp-address' outputs exactly the rp-address line(s).
PREFLIGHT_CMD = "show running-config | include ip pim rp-address"
# Present only after the fault is already active (wrong RP configured).
PREFLIGHT_FAULT_MARKER = "ip pim rp-address 10.0.0.2"
# Present only in the known-good state (correct RP configured).
PREFLIGHT_SOLUTION_MARKER = "ip pim rp-address 10.0.0.1"
