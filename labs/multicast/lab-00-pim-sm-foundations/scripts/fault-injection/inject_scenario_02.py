...
# Depth: scripts/fault-injection -> scripts -> lab-00-pim-sm-foundations -> multicast -> labs/
...
# Pre-flight: verify PIM state on the receiver-facing interface before injecting.
# When PIM is enabled, 'show running-config interface GigabitEthernet0/1' includes the pim line.
# When PIM is removed, that line is absent from the running-config.
PREFLIGHT_CMD = "show running-config interface GigabitEthernet0/1"
# Sentinel that cannot appear in a GigabitEthernet0/1 running-config — ensures the
# fault-marker branch of preflight() is never triggered (absence of SOLUTION_MARKER handles both).
PREFLIGHT_FAULT_MARKER = "__pim_fault_already_injected__"
# Present only in the known-good state (PIM enabled on the interface).
PREFLIGHT_SOLUTION_MARKER = "ip pim sparse-mode"
