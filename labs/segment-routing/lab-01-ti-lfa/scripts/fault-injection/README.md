# Fault Injection -- Segment Routing Lab 01 (TI-LFA)
...
- All nodes (R1, R2, R3, R4) must be **started** in EVE-NG
...
Injects a configuration change on R2. Observe the effect with:
...
Injects a configuration change on R2 affecting link L2 behavior. Observe with:
- `show bfd session`
...
Injects a change on R1. Observe domain-wide impact with:
- `show isis adjacency`
- `show isis fast-reroute summary`
