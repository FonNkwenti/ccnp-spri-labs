...
| Mapping server label conflict on a peer | Peer's SRGB too small for the advertised SID index; check `show segment-routing mpls connected-prefix-sid-map` |
...
! Verify the SRGB range — is SID index 50 (label 16050) within the configured block?
...
! Step 4: Check for conflict logs
R3# show segment-routing mpls state
! Look for SRGB conflict messages related to the mapping server entry
```
</details>
