#!/usr/bin/env python3
"""
Fault Injection: Scenario 03 — Remove set extcommunity color from RP_CE2_IN on R3.
...
# Pre-flight: verify the CE2 inbound route-policy includes the color community action.
# This is a removal fault — fault state has no positive indicator string,
# so PREFLIGHT_FAULT_MARKER is None and only the solution-state check is performed.
PREFLIGHT_CMD = "show rpl route-policy RP_CE2_IN"
# If this string is absent → not in solution state; bail out.
PREFLIGHT_SOLUTION_MARKER = "set extcommunity color"
