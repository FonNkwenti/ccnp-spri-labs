...
FAULT_COMMANDS = [
    "router bgp 65100",
    "address-family ipv4",
    "no neighbor 10.0.0.1 send-label",
]

# Pre-flight: confirm send-label is present on PE2 (solution state)
PREFLIGHT_CMD = "show running-config | section bgp"
# Solution state: send-label configured for PE1 neighbor on PE2
PREFLIGHT_SOLUTION_MARKER = "neighbor 10.0.0.1 send-label"


def preflight(conn) -> bool:
    output = conn.send_command(PREFLIGHT_CMD)
    if PREFLIGHT_SOLUTION_MARKER not in output:
        # IOS removes the line entirely on 'no neighbor X send-label'; it never
        # stores the 'no' form in running-config.  Absence of the solution marker
        # means either the fault is already injected or the lab is not in solution
        # state — either way, apply_solution.py is needed.
        print("[!] Pre-flight failed: lab not in expected pre-injection state.")
        print("    Either Scenario 03 is already active, or the lab is not in solution state.")
