...
# Pre-flight: verify MPLS is enabled on P1 Gi0/2 before injecting.
PREFLIGHT_CMD = "show mpls interfaces GigabitEthernet0/2"
# Present only in the known-good state (MPLS IP enabled).
PREFLIGHT_SOLUTION_MARKER = "Yes (ldp)"
# Present only after the fault is already injected (MPLS IP disabled, line shows "No").
# IOSv prints "GigabitEthernet0/2     No" when mpls ip is off.
PREFLIGHT_FAULT_MARKER = "GigabitEthernet0/2     No"
