...
| Script | Ticket |
|--------|--------|
| `inject_scenario_01.py` | Ticket 1 -- target device: R3 |
| `inject_scenario_02.py` | Ticket 2 -- target device: R2 |
| `inject_scenario_03.py` | Ticket 3 -- target device: R1 |
...
```bash
# 1. Restore to known-good state before each ticket
python3 apply_solution.py --host <eve-ng-ip>

# 2. Inject the fault for the ticket you are working on
python3 inject_scenario_01.py --host <eve-ng-ip>

# 3. Diagnose using only show commands (see workbook.md Section 9)
