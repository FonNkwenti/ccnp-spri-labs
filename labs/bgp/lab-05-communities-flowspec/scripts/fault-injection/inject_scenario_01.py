...
FAULT_COMMANDS = [
    "router bgp 65100",
    "address-family ipv4",
    "no neighbor 10.0.0.5 send-community both",
    "exit-address-family",
]

# Pre-flight: verify R4 has send-community both toward R5 before injecting.
PREFLIGHT_CMD = "show running-config | section address-family ipv4"
# Present only in the known-good (send-community configured) state.
PREFLIGHT_SOLUTION_MARKER = "neighbor 10.0.0.5 send-community both"
