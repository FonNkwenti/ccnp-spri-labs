...
Target:     R2 (PE East-1 -- iBGP neighbor 10.0.0.5 toward R5)
Injects:    Removes `neighbor 10.0.0.5 update-source Loopback0` from R2's
...
Result:     R5 sees the iBGP session from R2 stay in Active state permanently.
            R5's BGP table is empty -- no routes received from R2.
...
FAULT_COMMANDS = [
    "router bgp 65100",
    "no neighbor 10.0.0.5 update-source Loopback0",
]

# Pre-flight: read BGP running config on R2 to verify known-good state.
...
PREFLIGHT_FAULT_MARKER = "neighbor 10.0.0.5 update-source __FAULT_INJECTED__"
# If this string is absent -> not in solution state, bail out.
PREFLIGHT_SOLUTION_MARKER = "neighbor 10.0.0.5 update-source Loopback0"
...
def main() -> int:
    ...
    print("=" * 60)
    print("Fault Injection: Scenario 01 -- iBGP Session Stays in Active")
    print("=" * 60)
