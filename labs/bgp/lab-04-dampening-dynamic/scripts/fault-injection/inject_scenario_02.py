...
FAULT_COMMANDS = [
    "router bgp 65100",
    "no bgp listen range 10.99.0.0/24 peer-group DYN_CUST",
]

# Pre-flight: verify R2 has the listen range before injecting.
PREFLIGHT_CMD = "show running-config | section router bgp"
# Present only in the known-good (listen range configured) state.
PREFLIGHT_SOLUTION_MARKER = "bgp listen range 10.99.0.0/24 peer-group DYN_CUST"
...
def main() -> int:
    ...
    print("=" * 60)
    print("Fault Injection: Scenario 02 -- Dynamic Customer Session Stays Active")
    print("=" * 60)
