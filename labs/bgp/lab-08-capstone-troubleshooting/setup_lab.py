#!/usr/bin/env python3
"""
Lab Setup -- BGP Lab 08: BGP Comprehensive Troubleshooting -- Capstone II

Pushes pre-broken initial configs to all 7 lab devices via EVE-NG console ports.
Ports are discovered at runtime via the EVE-NG REST API -- no hardcoded ports needed.

This is a troubleshooting capstone. Initial configs contain 6 concurrent faults
spanning route-advertisement, multihoming policy, inter-domain security,
maximum-prefix safety, community propagation, and FlowSpec SAFI negotiation.

Use scripts/fault-injection/apply_solution.py to restore the known-good state
after the exercise.

Usage:
    python3 setup_lab.py --host <eve-ng-ip>
    python3 setup_lab.py --host <eve-ng-ip> --node R1,R2
    python3 setup_lab.py --host <eve-ng-ip> --reset --node R5
"""
...
# Depth: lab-08-capstone-troubleshooting/ -> bgp/ -> labs/
...
DEFAULT_LAB_PATH = "ccnp-spri/bgp/lab-08-capstone-troubleshooting.unl"
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Push pre-broken configs to lab nodes")
    ...
    parser.add_argument("--node", default=None,
                        help="Comma-separated device names to configure (e.g., R1,R5)")
    ...
    print("=" * 60)
    print(f"Lab Setup: BGP Lab 08 -- Capstone II (EVE-NG: {host})")
    print("NOTE: This topology contains 6 pre-planted faults.")
    if args.reset:
