# Fault Injection — Fast Convergence Lab 01
...
Target device: R1 — removes `nsf ietf` from IS-IS process
...
Target device: R5 — removes `bgp graceful-restart` from BGP process
...
Target device: R4 — removes `nsf ietf` from IS-IS process
