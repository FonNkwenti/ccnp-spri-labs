...
Ticket 2: Removes bgp graceful-restart from R5's BGP process.
Symptom: After R5 BGP restart, R1 does not enter GR helper mode — R1 immediately
         withdraws 192.0.2.0/24 because R5 no longer advertises GR capability.
"""
...
PREFLIGHT_SOLUTION_MARKER = "bgp graceful-restart"


def preflight(conn) -> bool:
    output = conn.send_command(PREFLIGHT_CMD)
    if PREFLIGHT_SOLUTION_MARKER not in output:
        print("[!] Pre-flight failed: bgp graceful-restart not present on R5 — lab not in expected pre-injection state.")
...
def main() -> int:
    ...
    print("=" * 60)
    print("Fault Injection: Scenario 02 — R5 BGP GR removed")
    print("=" * 60)
