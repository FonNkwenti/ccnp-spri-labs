...
Ticket 1: Removes nsf ietf from R1's IS-IS process.
Symptom: When R2 IS-IS restarts, R1 immediately removes R2's LSP from the LSDB
         (normal failure behavior) instead of entering GR helper mode.
"""
...
def preflight(conn) -> bool:
    ...
    if PREFLIGHT_SOLUTION_MARKER not in output:
        print("[!] Pre-flight failed: nsf ietf not present on R1 — lab not in expected pre-injection state.")
...
def main() -> int:
    ...
    print("=" * 60)
    print("Fault Injection: Scenario 01 — R1 IS-IS NSF removed")
    print("=" * 60)
