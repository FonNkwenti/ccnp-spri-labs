...
Ticket 3: Removes nsf ietf from R4's IS-IS process.
Symptom: When R4 IS-IS restarts, R1 and R3 immediately remove R4's LSP from the LSDB
         (normal failure) instead of entering GR helper mode for R4.
"""
...
PREFLIGHT_SOLUTION_MARKER = "nsf ietf"


def preflight(conn) -> bool:
    output = conn.send_command(PREFLIGHT_CMD)
    if PREFLIGHT_SOLUTION_MARKER not in output:
        print("[!] Pre-flight failed: nsf ietf not present on R4 — lab not in expected pre-injection state.")
...
def main() -> int:
    ...
    print("=" * 60)
    print("Fault Injection: Scenario 03 — R4 IS-IS NSF removed")
    print("=" * 60)
