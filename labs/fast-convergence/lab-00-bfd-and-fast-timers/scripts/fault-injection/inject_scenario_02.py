...
DEVICE_NAME = "R5"
FAULT_COMMANDS = [
    "no ip route 10.0.0.1 255.255.255.255 10.1.15.1",
    "no ip route 10.0.0.3 255.255.255.255 10.1.35.3",
]

# Pre-flight: check the static route to the first SP loopback to verify
# the lab is in the expected solution state before injecting.
# PREFLIGHT_CMD queries the routing table for R1's loopback host route.
PREFLIGHT_CMD = "show ip route 10.0.0.1"
# If this string is already present → fault already injected (route gone), bail out.
PREFLIGHT_FAULT_MARKER = "% Network not in table"
# If this string is absent → not in solution state, bail out.
PREFLIGHT_SOLUTION_MARKER = "10.0.0.1/32"
