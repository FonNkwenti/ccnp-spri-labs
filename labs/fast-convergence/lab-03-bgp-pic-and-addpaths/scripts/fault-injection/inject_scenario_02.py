...
Ticket 2: Removes bgp additional-paths select backup on R2. The global add-paths
          install is still present and two paths appear in the BGP table, but
          CEF does not install a backup next-hop because the "select backup"
          directive is missing.

Symptom: show ip cef 192.0.2.0 detail on R2 shows no backup next-hop despite
         show ip bgp 192.0.2.0 showing two installed paths.
"""
...
FAULT_COMMANDS = [
    "router bgp 65100",
    "no bgp additional-paths select backup",
]

PREFLIGHT_CMD = "show running-config | section router bgp"
PREFLIGHT_MARKER = "bgp additional-paths select backup"


def preflight(conn) -> bool:
    output = conn.send_command(PREFLIGHT_CMD)
    if PREFLIGHT_MARKER not in output:
        print("[!] Pre-flight failed: bgp additional-paths select backup not present on R2 — lab not in expected pre-injection state.")
...
def main() -> int:
    ...
    print("=" * 60)
    print("Fault Injection: Scenario 02 — R2 select backup missing")
    print("=" * 60)
