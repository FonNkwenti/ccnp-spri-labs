#!/usr/bin/env python3
"""
Lab Setup — IS-IS LFA and IP Fast Reroute (Fast Convergence Lab 02)
...
Initial configs are the lab-01 solutions (BFD + timers + NSF + BGP GR + NSR).
This lab adds per-prefix LFA, Remote LFA, and MPLS LDP on top — the student
configures those steps.
...
# Depth: lab-02-lfa-ip-fast-reroute/ -> fast-convergence/ -> labs/
...
DEFAULT_LAB_PATH = "ccnp-spri/fast-convergence/lab-02-lfa-ip-fast-reroute.unl"
...
def main() -> int:
    ...
    parser.add_argument("--reset", action="store_true",
                        help="Soft-reset before configuring: clear routing protocols and interface config")
    args = parser.parse_args()

    host = require_host(args.host)

    print("=" * 60)
    print(f"Lab Setup: IS-IS LFA and IP FRR (EVE-NG: {host})")
    print("=" * 60)
