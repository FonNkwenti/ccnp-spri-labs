...
Ticket 3: Removes fast-reroute per-prefix level-2 all from R3's IS-IS process.
          R3 becomes an LFA coverage gap in the mesh — R3 has no precomputed
          backup paths for any of its destinations.
Symptom: show isis fast-reroute summary on R3 shows "Prefixes protected: 0" or
         the command returns an error because fast-reroute is not configured at all.
"""
...
def preflight(conn) -> bool:
    ...
    if PREFLIGHT_SOLUTION_MARKER not in output:
        print("[!] Pre-flight failed: fast-reroute per-prefix level-2 all not present on R3 — lab not in expected pre-injection state.")
...
def main() -> int:
    ...
    print("=" * 60)
    print("Fault Injection: Scenario 03 — R3 LFA removed")
    print("=" * 60)
