...
Ticket 1: Replaces fast-reroute per-prefix level-2 all with a bogus prefix-list filter
          on R1's IS-IS process. The non-existent prefix-list causes IS-IS to compute zero
          LFA backups for all prefixes.
Symptom: show isis fast-reroute summary on R1 shows zero prefixes protected despite
         the configuration appearing correct at a glance.
"""
...
FAULT_COMMANDS = [
    "router isis",
    "no fast-reroute per-prefix level-2 all",
    "fast-reroute per-prefix level-2 route-map NONEXISTENT",
]

PREFLIGHT_CMD = "show running-config | section router isis"
PREFLIGHT_SOLUTION_MARKER = "fast-reroute per-prefix level-2 all"


def preflight(conn) -> bool:
    output = conn.send_command(PREFLIGHT_CMD)
    if PREFLIGHT_SOLUTION_MARKER not in output:
        print("[!] Pre-flight failed: fast-reroute per-prefix level-2 all not present on R1 — lab not in expected pre-injection state.")
...
def main() -> int:
    ...
    print("=" * 60)
    print("Fault Injection: Scenario 01 — R1 LFA prefix-list filter broken")
    print("=" * 60)
