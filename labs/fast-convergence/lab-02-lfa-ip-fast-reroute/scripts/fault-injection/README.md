# Fault Injection — Fast Convergence Lab 02
...
Target device: R1 — replaces `fast-reroute per-prefix level-2 all` with a bogus
`route-map NONEXISTENT` filter, causing zero LFA backups.
...
Target device: R3 — removes `fast-reroute per-prefix level-2 all` entirely,
leaving R3 with no LFA protection.
