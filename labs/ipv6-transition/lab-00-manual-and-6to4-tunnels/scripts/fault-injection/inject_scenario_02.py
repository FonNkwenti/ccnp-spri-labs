...
DEVICE_NAME = "R1"
FAULT_COMMANDS = [
    "no ipv6 route 2002::/16 Tunnel1",
]

# Pre-flight: read running config to verify the lab is in the expected solution
# state (the 6to4 static route is present) before injecting.
PREFLIGHT_CMD = "show running-config | include ipv6 route 2002"
# If this string is already present → fault already injected (route already missing
# would make the grep return nothing, so we check the inverse: the route is absent).
# IOS returns blank output when the route is missing; we detect injection by
# checking that the solution marker is absent in the preflight output.
PREFLIGHT_FAULT_MARKER = "!!FAULT_ALREADY_INJECTED_SENTINEL!!"
# If this string is absent → not in solution state, bail out.
PREFLIGHT_SOLUTION_MARKER = "ipv6 route 2002::/16 Tunnel1"
