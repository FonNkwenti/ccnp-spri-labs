# Topology — lab-00-single-level-isis

Three-router linear chain (R1–R2–R3) in a single IS-IS area `49.0001`, all
routers running `is-type level-1`. Two Ethernet links; no optional devices.
IOSv only.
...
| `lab-00-single-level-isis.unl` | EVE-NG native lab file — export from EVE-NG after building |
| `README.md` | This file |

## Importing the Lab

1. EVE-NG web UI → **File → Import** → select `lab-00-single-level-isis.unl`
2. Place at path `isis/lab-00-single-level-isis.unl` (matches `DEFAULT_LAB_PATH` in scripts)
