...
# Pre-flight: read R3's IS-IS process config to verify redistribution is present.
PREFLIGHT_CMD = "show running-config | section router isis"
# If this string is absent → not in solution state, bail out.
PREFLIGHT_SOLUTION_MARKER = "redistribute static ip"
# If this string is already present → fault already injected, bail out.
# After the fault, "redistribute static ip" is absent; there is no positive marker
# that replaces it. Use a sentinel that never appears in a clean config.
