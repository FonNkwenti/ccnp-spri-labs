# Fault Injection — IS-IS Lab 02: Dual-Stack Summarization
...
python3 apply_solution.py --host <eve-ng-ip> --reset --node R4
```
...
|----------|--------|---------------|
| Ticket 1 | R4 | `python3 inject_scenario_01.py --host <eve-ng-ip>` |
| Ticket 2 | R3 | `python3 inject_scenario_02.py --host <eve-ng-ip>` |
| Ticket 3 | R2 | `python3 inject_scenario_03.py --host <eve-ng-ip>` |

## Optional Flags

```bash
# Override lab auto-discovery (use if EVE-NG has multiple labs open)
python3 inject_scenario_01.py --host <eve-ng-ip> --lab-path ccnp-spri/isis/lab-02-dual-stack-summarization.unl

# Skip the pre-flight state check (use only if you are certain the lab is in solution state)
