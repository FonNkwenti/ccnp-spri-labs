#!/usr/bin/env python3
"""
Fault Injection: Scenario 01 — R3-R4 Neighbors Stuck in ExStart

Target:     R3 (Gi0/1 — link to R4, 10.1.34.0/24)
Injects:    Area mismatch on R3 Gi0/1: removes the subnet from Area 2 and places
            it into Area 0, causing R3 and R4 to disagree on the area for that link.
Fault Type: Area Mismatch

Result:     R3 and R4 neighbor state stuck in ExStart; routes from R1/R2/R5
            toward R4's loopback (172.16.4.0/24) disappear from the routing table.
...
FAULT_COMMANDS = [
    "router ospf 1",
    "no network 10.1.34.0 0.0.0.255 area 2",
...
PREFLIGHT_FAULT_MARKER = "network 10.1.34.0 0.0.0.255 area 0"
# If this string is absent → not in solution state, bail out.
PREFLIGHT_SOLUTION_MARKER = "network 10.1.34.0 0.0.0.255 area 2"
...
def main() -> int:
    ...
    print("=" * 60)
    print("Fault Injection: Scenario 01 — R3-R4 Neighbors Stuck in ExStart")
    print("=" * 60)
