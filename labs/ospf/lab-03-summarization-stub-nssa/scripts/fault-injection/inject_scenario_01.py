...
FAULT_COMMANDS = [
    ...
    "ipv6 prefix-list BLOCK_EXT_V6 seq 5 deny 2001:db8:66::/64",
    "ipv6 prefix-list BLOCK_EXT_V6 seq 10 permit ::/0 le 128",
    "router ospf 1",
    "distribute-list prefix BLOCK_EXT out",
    "router ospfv3 1",
    "address-family ipv6 unicast",
    "distribute-list prefix BLOCK_EXT_V6 out",
    "exit-address-family",
]

# Pre-flight: read running config on OSPF process to verify known-good state.
PREFLIGHT_CMD = "show running-config | section router ospf 1"
# If this string is already present -> fault already injected, bail out.
PREFLIGHT_FAULT_MARKER = "BLOCK_EXT"
# If this string is absent -> not in solution state, bail out.
PREFLIGHT_SOLUTION_MARKER = "redistribute static"
