...
# Remove the permit 20 catch-all from FILTER_R4_IN.
# On IOS, 'no route-map <name> permit <seq>' removes only that sequence.
FAULT_COMMANDS = [
    "no route-map FILTER_R4_IN permit 20",
]

# Scope to the route-map section so we get only FILTER_R4_IN entries.
PREFLIGHT_CMD = "show running-config | section route-map FILTER_R4_IN"

# Present in solution state (permit 20 exists); absent means fault is already active.
PREFLIGHT_SOLUTION_MARKER = "permit 20"

# Sentinel: no real string indicates this fault was injected. Positive duplicate
# detection is degraded -- absence of PREFLIGHT_SOLUTION_MARKER is the signal.
PREFLIGHT_FAULT_MARKER = "__SCENARIO_01_FAULT_NEVER_MATCHES__"
...
def preflight(conn) -> bool:
    ...
    if PREFLIGHT_FAULT_MARKER in output:
        print(f"[!] Pre-flight failed: fault marker already present (unexpected).")
        return False
    return True
