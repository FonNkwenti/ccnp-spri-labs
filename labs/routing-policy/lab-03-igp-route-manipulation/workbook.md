...
```
The route-map permits only the prefixes that should be leaked; everything else is blocked
...
are advertised into IS-IS. XR uses a `route-policy` applied at the IS-IS process level:
...
| RPL hierarchical IS-IS filtering | Apply nested `apply` RPL policies to IS-IS protocol advertisement |
...
| XR1 | IOS-XR RPL node — IS-IS L1-2 | XRv 9000 | xrv9k-fullk9-x.vrr-6.6.3 |
| XR2 | IOS-XR RPL node — IS-IS L2-only | XRv 9000 | xrv9k-fullk9-x.vrr-6.6.3 |
...
- iBGP full mesh in AS 65100 across R1, R2, R3, XR1, XR2 (peer-groups / neighbor-groups, Loopback0 source, next-hop-self)
...
- RPL hierarchical IS-IS advertisement filter on XR1 (ISIS_L1_FILTER_PARENT/CHILD)
...
- After applying, confirm that 172.16.1.0/24 is absent from R2's IP routing table but is
  still present in R2's OSPF LSDB as a Type 5 (external) LSA — the key distinction between
  distribute-list and filter-list.
- R1 and R3 must still have 172.16.1.0/24 in their routing tables.

**Verification:** `show ip route 172.16.1.0` on R2 must return no result. `show ip ospf database external` on R2 must still show the LSA for 172.16.1.0/24. `show ip route 172.16.1.0` on R1 and R3 must still show the route.
...
**Verification:** `show isis neighbors` on R2 must show R1 and R3 as L2, and XR1 as L1. `show isis neighbors` on XR1 must show R2 as L1 and XR2 as L2. XR1's loopback 10.0.0.5/32 must NOT yet appear in R3's IS-IS L2 routing table (it is only in L1 until the leak in Task 5).
...
- On R2's IS-IS process, add a statement that redistributes IS-IS level-1 routes into level-2
  using the `LEAK_L1_TO_L2` route-map.
- Verify that 10.0.0.5/32 and 172.16.11.0/24 now appear in the IS-IS L2 routing table on R3
  and XR2, but that XR1's Loopback1 subnet (which is NOT in LEAK_FROM_L1) would be blocked
  by the implicit deny if it were a different prefix.

**Verification:** `show ip route isis` on R3 must include 10.0.0.5/32 and 172.16.11.0/24 with IS-IS L2 tags. `show route ipv4 isis` on XR2 must also include these prefixes. `show ip isis database detail` on R2 must confirm the L2 LSP for R2 now carries 10.0.0.5/32 and 172.16.11.0/24.

---

### Task 6: XR1 IS-IS Filter Using RPL Hierarchical `apply`
...
- Write a parent route-policy named `ISIS_L1_FILTER_PARENT` that calls `ISIS_L1_FILTER_CHILD`
  via the `apply` statement. This demonstrates the `apply` mechanism at the IS-IS protocol
  level — the same hierarchical composition shown in lab-02 for BGP, now applied to IS-IS.
- Apply `ISIS_L1_FILTER_PARENT` as the outbound route-policy on XR1's IS-IS address-family.
  Contrast this with the IOS equivalent (`distribute-list prefix-list X out` under `router isis`).
- Verify that XR1 advertises only 10.0.0.5/32 and 172.16.11.0/24 into IS-IS; no other prefixes
  from XR1 (e.g., learned BGP routes) appear in the IS-IS LSDB as originating from XR1.

**Verification:** `show isis database detail XR1` on R2 or R3 must show only 10.0.0.5/32 and 172.16.11.0/24 as reachable via XR1. `show rpl policy ISIS_L1_FILTER_PARENT` on XR1 must show the `apply ISIS_L1_FILTER_CHILD` statement.
...
% Network not in table                          ! ← route blocked from RIB by BLOCK_MGMT
...
i L2  10.0.0.5/32 [115/30] via 10.1.23.2, GigabitEthernet0/0   ! ← XR1 Lo0 leaked to L2
i L2  172.16.11.0/24 [115/30] via 10.1.23.2, GigabitEthernet0/0 ! ← XR1 Lo1 leaked to L2
! 10.2.1.0/24 (R2's area-1 loopback) must NOT appear here via IS-IS

RP/0/0/CPU0:XR2# show route ipv4 isis
i L2  10.0.0.5/32 [115/30] via 10.1.56.5, GigabitEthernet0/0/0/1  ! ← via XR1 direct L2
i L2  172.16.11.0/24 [115/30] via 10.1.56.5, GigabitEthernet0/0/0/1
```

### Task 6 — XR1 IS-IS RPL Filter Verification

```
RP/0/0/CPU0:XR1# show rpl policy ISIS_L1_FILTER_PARENT
route-policy ISIS_L1_FILTER_PARENT
  apply ISIS_L1_FILTER_CHILD                  ! ← hierarchical apply visible here
end-policy

R2# show isis database detail XR1.00-00
IS-IS SP (Level-1) Link State Database:
LSPID                 LSP Seq Num  LSP Checksum  LSP Holdtime  ATT/P/OL
XR1.00-00           * 0x0000000a   0x3c2a        1197          1/0/0
  ...
  IPv4 Reachability: 10.0.0.5/32 Metric: 10    ! ← only XR1 loopbacks present
  IPv4 Reachability: 172.16.11.0/24 Metric: 10 ! ← XR1 Lo1
  ! No BGP-learned routes from XR1 in IS-IS LSDB
```
...
! IOS — selective L1-to-L2 leak
router isis SP
  redistribute isis level-1 into level-2 route-map <name>
...
> **Exam tip:** A missing `route-map` on `redistribute isis level-1 into level-2` means ALL L1
...
! Child policy — the actual filter logic
...
! Parent policy — uses apply for hierarchical composition
route-policy ISIS_FILTER_PARENT
  apply ISIS_FILTER_CHILD
end-policy

! Apply to IS-IS process (XR)
router isis SP
  address-family ipv4 unicast
    route-policy ISIS_FILTER_PARENT out
```
...
| `show ip isis topology` | Confirm redistribution paths through R2 |
...
| XR1 loopbacks missing from L2 routing table on R3 | `redistribute isis level-1 into level-2` missing or route-map implicit deny |
...
| R2 still sees 172.16.1.0/24 in routing table | `distribute-list prefix BLOCK_MGMT in` not applied under `router ospf 1` |
...
```bash
R3# show ip route isis
! expect: i L2 10.0.0.5/32 and i L2 172.16.11.0/24 both present

XR2# show route ipv4 isis
! expect: same two prefixes via IS-IS L2
```
</details>

---

### Task 6: XR1 IS-IS RPL Hierarchical Filter
...
! expect: "apply ISIS_L1_FILTER_CHILD" visible in body

R2# show isis database detail XR1.00-00
! expect: only 10.0.0.5/32 and 172.16.11.0/24 in XR1's LSP
```
</details>
...
```bash
! R2 — update OSPF_TO_ISIS (deny seq 10 already present from lab-02)
...
- [ ] Task 1: BLOCK_MGMT prefix-list applied as distribute-list in on R2's OSPF; 172.16.1.0/24 absent from R2 RIB, present in LSDB
- [ ] Task 2: R2's Loopback1 in area 1 stub; area 0 filter-list blocks Type 3 LSA for 10.2.1.0/24 from R1/R3
- [ ] Task 3: R1 Gi0/0 and Gi0/2 have prefix-suppression; adjacencies FULL; transit subnets absent from R1's Type 1 LSA
- [ ] Task 4: R2 is level-1-2; G0/2 is circuit-type level-1; XR1 is level-1-2; R2↔XR1 L1 adjacency up; R2↔R1/R3 L2 adjacencies intact
...
- [ ] Task 6: ISIS_L1_FILTER_CHILD and ISIS_L1_FILTER_PARENT defined on XR1; parent applied to IS-IS address-family out; XR1 LSDB shows only P_XR1_LOOPBACKS prefixes
