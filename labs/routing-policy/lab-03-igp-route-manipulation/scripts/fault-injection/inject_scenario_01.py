...
# Fault: remove the correct BLOCK_MGMT and replace with an overly broad /16 deny.
# The broader deny blocks prefixes beyond just 172.16.1.0/24, causing valid routes
# to be filtered from the RIB by the distribute-list.
FAULT_COMMANDS = [
    "no ip prefix-list BLOCK_MGMT",
    "ip prefix-list BLOCK_MGMT seq 5 deny 172.16.0.0/16",
    "ip prefix-list BLOCK_MGMT seq 10 permit 0.0.0.0/0 le 32",
]

# Pre-flight: verify BLOCK_MGMT has the correct /24 deny before injecting.
PREFLIGHT_CMD = "show running-config | section ip prefix-list BLOCK_MGMT"
# Present only after fault is injected (the broad /16 deny).
PREFLIGHT_FAULT_MARKER = "deny 172.16.0.0/16"
# Present in the known-good solution state (the correct /24 deny).
PREFLIGHT_SOLUTION_MARKER = "deny 172.16.1.0/24"
