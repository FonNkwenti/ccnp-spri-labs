...
# Config-mode commands: replace the correct /24 entry with the wrong /25.
FAULT_CONFIG_COMMANDS = [
    "no ip prefix-list PFX_PREFER_172_20_4 seq 5 permit 172.20.4.0/24",
    "ip prefix-list PFX_PREFER_172_20_4 seq 5 permit 172.20.4.0/25",
]

# Exec-mode soft-reset so R3 re-evaluates inbound policy from R4.
FAULT_EXEC_COMMANDS = [
    "clear ip bgp 10.1.34.4 soft in",
]

# Pre-flight: inspect the prefix-list via running-config.
PREFLIGHT_CMD = "show running-config | section ip prefix-list PFX_PREFER_172_20_4"
# Present only after fault is injected (the wrong /25 mask).
PREFLIGHT_FAULT_MARKER = "permit 172.20.4.0/25"
# Present in the known-good solution state (the correct /24 mask).
PREFLIGHT_SOLUTION_MARKER = "permit 172.20.4.0/24"
...
def main() -> int:
    ...
    try:
        ...
        conn.save_config()
        print("[*] Triggering soft inbound reset to R4 ...")
